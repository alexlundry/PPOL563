...
animate(p2)
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/gapminder1.gif")
```
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/gapminder2.gif")
```
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/gapminder3.gif")
```
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/gapminder4.gif")
```
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/gapminder5.gif")
```
...
Thanks to Gina Reynolds for the [demo](https://evamaerey.github.io/little_flipbooks_library/racing_bars/racing_barcharts.html#35)
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/bar_race1.gif")
```

### Another Racing Bar Chart demo

This one comes from [amrrs]("https://github.com/amrrs/animated_bar_charts_in_R") and the associated article at [R Bloggers]("https://www.r-bloggers.com/2020/01/how-to-create-bar-race-animation-charts-in-r/")
...
anim_save("~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/bar_race2.gif")
```
...
We'll be showing 2016 House election results in NC.  This is adapted from [here]("https://mdsr-book.github.io/mdsr2e/ch-spatial.html")

```{r}
install.packages("fec16")
library(fec16)

nc_results <- results_house %>% 
...
Now we need a congressional district shapefile for the 114th Congress.  We 

```{r}
src <- "https://cdmaps.polisci.ucla.edu/shp/districts114.zip"
# dsn_districts <- usethis::use_zip(src, destdir = fs::path("data_large"))
dsn_districts <- "~/Dropbox (Personal)/Teaching/PPOL 563/Week 9 - Animation & Interaction/districtShapes/"
...
We need to merge in the election data with the shape file.  The simplest way to do that is to use an inner_join().  Here we merge the nc_shp polygons with the nc_results election data frome using the district as the key.  
...
We can use Leaflet.  First we will define a color palette over the values [0,1] that ranges from red to blue.
