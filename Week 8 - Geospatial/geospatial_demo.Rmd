...
Once we have the basemap, we can layer on other maps.  Here we've layered on a polygon state border by filtering the state map data to just Virginia, and then we layer on our SF based map. 

```{r}
va_plot2 <- ggmap(va_basemap) +
   geom_polygon(data=map_data("state") %>% filter(region == "virginia"), 
                aes(x=long, y=lat, group=group)) +
   geom_sf(data = va_dma_20, aes(fill = biden_percent), 
           color = "light grey", alpha = 0.9, inherit.aes = F) +
   scale_fill_distiller(palette = "Blues") +
   coord_sf(crs = st_crs(4326))

va_plot2 <- ggplot(va_dma_20) +
   geom_sf(aes(fill = biden_percent), color = "light grey", alpha = 0.9) +
   geom_polygon(data=map_data("state") %>% filter(region == "virginia"), 
